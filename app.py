...
df_global = pd.read_csv(df_path)  # données brutes

@app.route("/predict", methods=["POST"])
def predict():
    data = request.json
    sk_id_curr = data.get("SK_ID_CURR")
    data_modified = data.get("data", None)

    try:
        if data_modified is not None:
            # ✅ Correction ici : transformer dict en DataFrame avec une seule ligne
